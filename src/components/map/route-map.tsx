...
export function RouteMap({ stops, className = "", height = "400px" }: RouteMapProps) {
  ...
  useEffect(() => {
    ...
    map.on("load", () => {
      // Force canvas to recalculate its size after the DOM has fully painted
      map.resize();
  ...
  return (
    ...
    >
      {/* Explicit dimensions on the container so Mapbox can measure it */}
      <div
        ref={containerRef}
      ...
      {ready && (
        <div
          ...
          className="bg-white/90 backdrop-blur rounded-lg shadow px-3 py-2 flex items-center gap-4 text-xs font-medium"
        >
