...
export default async function ReportsPage({
  ...
  const onTimeRate = attendanceLogs.length > 0
      ...
      <div className="flex items-center justify-between flex-wrap gap-4">
        <div>
          <h1 className="text-2xl font-bold flex items-center gap-2">
            <FileBarChart className="h-6 w-6" /> Weekly Transportation Report
          </h1>
          <p className="text-muted-foreground text-sm mt-1">Week of {weekLabel}</p>
        </div>
        <ReportControls weekOffset={weekOffset} weekLabel={weekLabel} dates={dates} dayLabels={dayLabels} />
      </div>
            ...
            <table className="w-full text-sm border-collapse">
              <thead>
                <tr className="border-b bg-muted/50">
                  ...
                  <th className="text-left px-4 py-3 font-semibold whitespace-nowrap hidden md:table-cell">School</th>
                  {dayLabels.map((label, i) => (
                    <th key={i} className="text-center px-3 py-3 font-semibold whitespace-nowrap text-xs">
                      {label}
                    </th>
                  ))}
                  <th className="text-left px-4 py-3 font-semibold whitespace-nowrap hidden lg:table-cell">Dismissal</th>
                  <th className="text-left px-4 py-3 font-semibold whitespace-nowrap hidden lg:table-cell">Guardian</th>
                  <th className="text-left px-4 py-3 font-semibold whitespace-nowrap hidden lg:table-cell">Phone</th>
                </tr>
              </thead>
              <tbody>
                {children.map((child, idx) => {
                  ...
                  return (
                    <tr key={child.id} className={cn("border-b", idx % 2 === 0 ? "" : "bg-muted/20")}>
                      <td className="px-4 py-2.5 font-medium whitespace-nowrap">{child.fullName}</td>
                      <td className="px-4 py-2.5 text-muted-foreground whitespace-nowrap hidden md:table-cell text-xs">{child.school.name}</td>
                      {dates.map((date) => {
                        const log = childLogs.get(date);
                        if (!log) {
                          return (
                            <td key={date} className="px-3 py-2.5 text-center text-xs text-muted-foreground">—</td>
                          );
                        }
                      ...
                      <td className="px-4 py-2.5 whitespace-nowrap hidden lg:table-cell text-xs">{child.school.dismissalTime}</td>
                      <td className="px-4 py-2.5 whitespace-nowrap hidden lg:table-cell text-xs">{child.guardianName}</td>
                      <td className="px-4 py-2.5 whitespace-nowrap hidden lg:table-cell text-xs font-mono">{child.guardianPhone}</td>
                    </tr>
                  );
                })}
              </tbody>
